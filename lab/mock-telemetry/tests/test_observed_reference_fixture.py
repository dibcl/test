...
class ObservedReferenceFixtureTests(unittest.TestCase):
    ...
    def test_batch_counts_match_filtered_observation(self) -> None:
        ...
        self.assertEqual([item["observed_count"] for item in batches], [10, 6, 9])
        self.assertEqual([item["retained_count"] for item in batches], [10, 5, 9])
        self.assertEqual([len(item["softwares"]) for item in batches], [10, 5, 9])
    ...
    def test_sensitive_software_is_not_in_retained_rows(self) -> None:
        names = {
            row["name"]
            for batch in self.value["batches"]
            for row in batch["softwares"]
